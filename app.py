...
MONTH_THEMES = {
    ...
    2: {'color': '#F48FB1', 'name': 'Pastel Love'}, # Feb: Pastel Pink
...
def finishers_canvas(year, month):
    ...
    # SAFETY: Handle cases where monthly_stats might not exist yet
    finishers.sort(key=lambda x: (x.get('monthly_stats') or {}).get(badge_key, 0), reverse=True)
    
    # FIXED: This line was crashing before because MONTH_THEMES was missing in the old file
