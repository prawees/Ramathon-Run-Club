...
def get_aqi():
    """Fetches AQI with Fallback"""
    try:
        # 1. Try Ratchathewi (Precise)
        url = f"https://api.waqi.info/feed/ratchathewi/?token={AQI_TOKEN}"
        r = requests.get(url, timeout=3)
        data = r.json()
        
        if data.get('status') != 'ok':
            # 2. Fallback to Bangkok (General) if Ratchathewi fails
        ...
        if data.get('status') == 'ok':
            ...
            elif aqi <= 100: return {'val': aqi, 'status': 'Moderate', 'color': '#FFC107', 'msg': 'Acceptable. Enjoy your run.'}
            elif aqi <= 150: return {'val': aqi, 'status': 'Unhealthy (Sens.)', 'color': '#FF9800', 'msg': 'Sensitive groups should reduce running.'}
            else: return {'val': aqi, 'status': 'Unhealthy', 'color': '#F44336', 'msg': 'Bad air! Use a treadmill! 😷'}
    except:
        pass
    
    # 3. Always return SOMETHING so UI doesn't break
    return {'val': 'N/A', 'status': 'No Data', 'color': '#9E9E9E', 'msg': 'Check your API Token.'}
